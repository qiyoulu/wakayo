...
def promote_entry(
    ...
    """Mark an entry as promoted and append it to ~/MEMORY.md.

    The entry is appended as a §-delimited block so it round-trips through
    the memory tool's parser. If memory_md_path is None, uses ~/MEMORY.md.
    """
    ...
    out_path = memory_md_path or Path.home() / "MEMORY.md"
    block = _promotion_block(row)

    _append_atomic(out_path, block)
    return row
